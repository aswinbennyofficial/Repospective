...
// Function to dynamically set up language-specific action
const getLanguageSetupStep = (language) => {
    let setupAction = '';

    switch (language) {
        case 'javascript':
            setupAction = 'actions/setup-node@v2';
            break;
        case 'golang':
            setupAction = 'actions/setup-go@v2';
            break;
        case 'python':
            setupAction = 'actions/setup-python@v2';
            break;
        default:
            setupAction = '';
    }

    return setupAction ? { name: 'Set up language', uses: setupAction } : {};
};
